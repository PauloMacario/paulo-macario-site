...
class CreateWithInstallmentsOtherMonth
{
    protected $data;
    ...
    public function execute()
    {
        ...
        $startBaseDate = Carbon::createFromFormat('Y-m-d', $this->debt->date);

        $debtDay = $startBaseDate->format('d');
       
       
        if ($debtDay >= $this->debt->paymentType->processing_day) {
            $startBaseDate->addMonth();
        }

        if ($this->debt->paymentType->payment_day < $this->debt->paymentType->processing_day) {
            $startBaseDate->addMonth();
        }

        $startDateGer = $startBaseDate->format('Y-m') . '-' . $this->debt->paymentType->payment_day;

        $startDate = Carbon::createFromFormat('Y-m-d', $startDateGer);
